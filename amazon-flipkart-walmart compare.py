...
def csv_conversion(obj, filename, identity):
    ...
    if identity == 'laptop':
        laptop_fields = ['Title', 'Image', 'Walmart Price', 'Walmart Rating', 'Walmart Voted', 'Walmart Link', 'Amazon Price', 'Amazon Rating', 'Amazon Voted', 'Amazon Link']
        ...
        with open(laptop_file_name, 'w') as laptop_csv_file:
            ...
            for l_data in obj:
                ...
                try:
                    ...
                    a_rating = ''
                laptop_csv_writer.writerow([l_data.title, l_data.image, l_data.walmart_price, w_rating, w_voted, l_data.walmart_link, l_data.amazon_price, a_rating, l_data.amazon_voted, l_data.amazon_link])
    
    elif(identity == 'mobile'):
        mobile_fields = ['Title', 'Image', 'Walmart Price', 'Walmart Rating', 'Walmart Voted', 'Walmart Link', 'Flipkart Price', 'Flipkart Rating', 'Flipkart Link', 'Flipkart voted']
        ...
        with open(mobile_file_name, 'w') as mobile_csv_file:
            ...
            for m_data in obj:
                ...
                mobile_csv_writer.writerow([m_data.title, m_data.image, m_data.walmart_price, w_rating, w_voted, m_data.walmart_link, m_data.flipkart_price, m_data.flipkart_rating, m_data.flipkart_link, f_voted])
...
def collect_data_from_walmart(url, identity):
    ...
    for product in products:
        ...
        link = 'https://www.walmart.com' + product.find('a', class_ = 'product-title-link line-clamp line-clamp-2')['href']
        try:
            rating = product.find('span', class_ = 'stars-container')['alt']
        except:
            rating = ''
        try:
            price = product.find('span', class_ = 'price display-inline-block arrange-fit price price-main').span['aria-label']
        except:
            price = ''
        img = product.img
...
def collect_data_from_amzn(walmart_product):
    ...
    for p in walmart_product:
        ...
        products = soup.find_all('div', class_='sg-col-20-of-24 s-result-item sg-col-0-of-12 sg-col-28-of-32 sg-col-16-of-20 sg-col sg-col-32-of-36 sg-col-12-of-16 sg-col-24-of-28')[:1]
        ...
        for product in products:
            ...
            try:
                ...
                p.amazon_title = ''
            try:
                p.amazon_link = 'https://www.amazon.com'+product.find('h5', class_='a-color-base s-line-clamp-2').a['href']
            except:
                p.amazon_link = ''
